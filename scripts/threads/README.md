...
- スケジュール: **毎日10回 JST（価値提供8 : 誘導2）**
  - **07:00 / 10:00 / 17:00 / 21:00** … 価値発信（固定プール・リンクなし・約1/3は失敗談）
  - **08:00 / 15:00 / 18:30 / 22:30** … 数字ネタ（[`num_posts.py`](./num_posts.py) が複利・逆算・取り崩し・開始時期を自動計算生成。同一投稿の再登場は最短16日）
...
| slot | 時刻(JST) | 種類 |
|------|-----------|------|
| 0 | 07:00 | value（固定プール・リンクなし） |
| 1 | 08:00 | num（数字ネタ自動生成・リンクなし） |
| 2 | 10:00 | value |
...
数字ネタ（`kind=num`）は [`num_posts.py`](./num_posts.py) がパラメータから毎回生成するため追記不要。形式やパラメータ空間を増やす場合はそちらを編集する。  
