...
- 絵文字は基本なし（ハート／✨系は禁止）
- 投資・NISA・URL禁止
- **一日の整合性**: `casual_consistency.py` が「今日ジム行った/サボった」「出社/在宅」「朝/昼/夜」「曜日」の衝突を避ける
- **毎日の記憶**: [`day_plans.json`](./day_plans.json) にその日の全枠を保存。前日の flags を引き継ぎ（例: 昨日トレしてないのに「昨日の脚トレ」は出さない）
